...
echo -e "${CYAN}╔══════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║   AI Climate Finance / Carbon Registry           ║${NC}"
echo -e "${CYAN}╚══════════════════════════════════════════════════╝${NC}"
echo ""
...
pkill -9 -f "AIClimateFinanceCarbonRegistry/backend.*nodemon"        2>/dev/null || true
pkill -9 -f "AIClimateFinanceCarbonRegistry/backend.*server\.js"     2>/dev/null || true
pkill -9 -f "AIClimateFinanceCarbonRegistry/frontend.*react-scripts" 2>/dev/null || true
...
psql -h ${DB_HOST:-localhost} -p ${DB_PORT:-5432} -U ${DB_USER:-postgres} -tc "SELECT 1 FROM pg_database WHERE datname = '${DB_NAME:-carbon_registry}'" 2>/dev/null | grep -q 1 || \
  psql -h ${DB_HOST:-localhost} -p ${DB_PORT:-5432} -U ${DB_USER:-postgres} -c "CREATE DATABASE ${DB_NAME:-carbon_registry}" 2>/dev/null || \
  createdb -h ${DB_HOST:-localhost} -p ${DB_PORT:-5432} -U ${DB_USER:-postgres} ${DB_NAME:-carbon_registry} 2>/dev/null || true
